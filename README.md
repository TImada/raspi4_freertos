...
This FreeRTOS porting uses UART2. The sample application is designed to be launched by u-boot and to operate together with 64-bit Linux.

Implementation is based on another FreeRTOS porting for Raspberry Pi 3 by eggman [1] (many thanks to him).  

The sample program runs on the CPU core #3 on your Raspberry Pi 4B board. A specified memory region (0x20000000 - 0x207FFFFF) is dedicated to the sample application. Modify `FreeRTOS/Demo/CORTEX_A72_64-bit_Raspberrypi4/uart/src/raspberrypi4.ld` if you want to change the memory usage.
...
```
$ sudo ./uart.bin /path/to/sd_boot_partition/
```
...
You will obtain `bcm2711-rpi-4-b-rtos.dtb`. Copy it to your a SD card partition where Linux kernel will load it. You may need to rename the binary dtb file or to modify a board configuration file such as `config.txt` to make it possible for Linux kernel to load it properly.


#### Sample program compilation  
You need to compile it with a macro `__LINUX__`. This macro adds a special function to avoid GIC configuration change by Linux.

```
$ cd Demo/CORTEX_A72_64-bit_Raspberrypi4/uart
$ make CROSS=aarch64-none-elf- -D__LINUX__
```
(`CROSS` must be changed depending on a compiler you installed)

(3) Copy the obtained binary to your SD card  
Same as 4-(1) above.
...
bootelf 0x28000000
```
But you will see only a string
```
Waiting until Linux starts booting up ...
```
on UART2 until you launch Linux.
...
run bootcmd
```
on the u-boot prompt. You will see Linux boot process output on UART1 and FreeRTOS UART output on UART2.
